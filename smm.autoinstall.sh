#!/bin/bash
# smm Masternode Setup Script V1 for Ubuntu 16.04 LTS
#
# Script will attempt to autodetect primary public IP address
# and generate masternode private key unless specified in command line
#
# Usage:
# bash smm.autoinstall.sh
#
...
function stop_daemon {
    if pgrep -x 'smmd' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop smmd${NC}"
        smm-cli stop
        sleep 30
        if pgrep -x 'smmd' > /dev/null; then
            echo -e "${RED}smmd daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 smmd
            sleep 30
            if pgrep -x 'smmd' > /dev/null; then
                echo -e "${RED}Can't stop smmd! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${GREEN} ------- smm MASTERNODE INSTALLER v2.0.0--------+
 |                                                  |
 |                                                  |::
 |       The installation will install and run      |::
 |        the masternode under a user smm.         |::
 |                                                  |::
...
cp -p -r smm-cli /usr/local/bin

  smm-cli stop
 sleep 5
 #Create datadir
 if [ ! -f ~/.smm/smm.conf ]; then 
 	sudo mkdir ~/.smm
 fi
clear
echo -e "${YELLOW}Creating smm.conf...${NC}"
...
#Setting auto start cron job for smm
if ! crontab -l | grep "smmauto.sh"; then
    (crontab -l ; echo "*/5 * * * * /root/smmauto.sh")| crontab -
fi
...
======================================================================== \a"
echo -e "${GREEN}smm_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}smm_mn1${NC} - with your desired masternode name (alias)
    ${GREEN}TxId${NC} - with Transaction Id from masternode outputs
    ${GREEN}TxIdx${NC} - with Transaction Index (0 or 1)
     Remember to save the masternode.conf and restart the wallet!
To introduce your new masternode to the smm network, you need to
...
========================================================================
To view masternode configuration produced by this script in smm.conf:
${GREEN}cat ~/.smm/smm.conf${NC}
Here is your smm.conf generated by this script:
-------------------------------------------------${GREEN}"
echo -e "${GREEN}smm_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
cat ~/.smm/smm.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit smm.conf, first stop the smmd daemon,
then edit the smm.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the smmd daemon back up:
to stop:              ${GREEN}smm-cli stop${NC}
to start:             ${GREEN}smmd${NC}
to edit:              ${GREEN}nano ~/.smm/smm.conf${NC}
to check mn status:   ${GREEN}smm-cli masternode status${NC}
========================================================================
